...
function Row({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="w-14 shrink-0 text-xs text-neutral-400">{label}</span>
      <button
        onClick={() => set(clamp(value - 1))}
        data-tooltip={t("前一个八拍")}
        aria-label={t("前一个八拍")}
        className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 text-neutral-300 hover:bg-white/10"
      >
        −
      </button>
      <span className="w-24 text-center text-sm tabular-nums text-white">
        {language === "en" ? `8-count ${value + 1}` : `第 ${value + 1} 个八拍`}
      </span>
      <button
        onClick={() => set(clamp(value + 1))}
        data-tooltip={t("后一个八拍")}
        aria-label={t("后一个八拍")}
        className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 text-neutral-300 hover:bg-white/10"
      >
        +
      ...
      <button
        onClick={setToNow}
        title={t("设为当前播放到的八拍")}
        className="ml-auto flex shrink-0 items-center gap-1 whitespace-nowrap rounded-lg bg-white/5 px-2.5 py-1.5 text-xs text-neutral-200 hover:bg-white/10"
      >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <motion.div
        ...
        className="w-[min(460px,calc(100vw-2rem))] rounded-2xl border border-white/10 bg-neutral-900 p-4 shadow-2xl sm:p-5"
      >
        ...
        <label className="mb-1 block text-xs text-neutral-400">{t("段落名称")}</label>
        <input
          autoFocus
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder={t("如：副歌 / 第一段")}
        ...
        <div className="space-y-2.5">
          <Row label={t("起点")} value={startSeg} set={setStartSeg} clamp={clampS} setToNow={() => setStartSeg(clampS(currentBeatIndex))} />
          <Row label={t("终点")} value={endSeg} set={setEndSeg} clamp={clampS} setToNow={() => setEndSeg(clampS(currentBeatIndex))} />
        </div>
        ...
        <p className="mt-3 text-xs text-neutral-500">
          {valid ? (
            <>
              {language === "en" ? (
                <>
                  <span className="font-medium text-neutral-300">{len}</span>{" "}
                  {len === 1 ? "8-count" : "8-counts"}
                </>
              ) : (
            ...
            <span className="text-red-400">{t("起点须不晚于终点")}</span>
          )}
        </p>
